...
- **상세 조건 다중 필터 (Advanced Condition Filters)**:
  - 상단 필터바의 `[🔍 조건 필터 ▼]` 토글 버튼으로 열고 닫을 수 있는 상세 조건 다중 필터 패널을 추가했습니다.
  - 7개 항목(`초과하락 %p 이상`, `전세가율 % 이상`, `거래량비 배 이상`, `거래 건수 건 이상`, `학원가 곳 이상`, `역세권 m 이내`, `연식 년 이하`)에 최소/최대값을 입력하여 다중 조건 필터링이 가능합니다.
  - 여러 조건 적용 시 AND 로직으로 통과 단지만 선별하며, 특정 항목의 값이 빈칸(`null` 등)인 매물은 조건 적용 시 자동 제외됩니다.
  - 표 상단에 `🔍 조건 N개 적용 · M곳 표시` 요약바 및 `[🔄 조건 초기화]` 버튼을 제공하며, 모든 입력 조건과 패널 개폐 상태는 브라우저 LocalStorage에 기억됩니다.
