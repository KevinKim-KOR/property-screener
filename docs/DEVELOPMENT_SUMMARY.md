...
- **11개 정제된 테이블 컬럼 체계**:
  ```
  순위 | 지역명 | 단지명 (동 / 층) | 평형 | 현재가 | 전고점 | 최근 1개월 | 최근 3개월 | 최근 6개월 | 퀀트점수 | 매물확인
  ```
  - **`전고점` 표기 개선**: 기존 취소선(`line-through`)을 제거하여 명확한 가격 정보로 표시합니다.
  - **한국식 직관적 시세 추세(1M / 3M / 6M) 컬러 표기**:
    - **상승 추세**: `▲ +3.5%` (적색 강조 `#f87171`)
    - **하락 추세**: `▼ -1.8%` (청색 표시 `#60a5fa`)
    - **보합/변동없음**: `0.0%` (회색 표기 `#94a3b8`)
- **실시간 인터랙티브 평형 & 매매가 필터**:
