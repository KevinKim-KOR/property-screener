...
> **(2026-09-07 개정) Φ 매핑은 중앙값 기준으로 표준화한다.**
>
> ```
> 이전:  Raw_z = (raw - mean)   / std
> 개정:  Raw_z = (raw - median) / std
> ```
>
> 근거: V11 이 점수 중앙값을 검사하므로 매핑도 중앙값을 기준으로 해야
> 전제와 구현이 일치한다. 평균 기준은 원점수가 비대칭일 때 중앙값이
> 구조적으로 50 에서 벗어나, 정상 실행이 반려된다
> (실측: 중앙값 46.6 / 44.2 로 두 차례 run 반려됨).
>
> 표준화 중심만 바뀌므로 순위는 사실상 그대로다(실측 167건 중 2건만 이동,
> 동점 처리 차이). 점수 값만 이동한다.
...
ui:
  area_labels:            # §16.2 표기 변환 (C10)
    ...
    A59:   "25평형 · 전용 59㎡"
    A84:   "34평형 · 전용 84㎡"
  ※ v4.3 에서 `34평 / 84㎡` 형식으로 변경됨 (SCORING_DESIGN_v4.3.md §2-①)
    A114:  "44평형 · 전용 114㎡"
    A135P: "50평형 이상"
...
| 90점 이상 비율 | **12% + 2×표본흔들림 이하** (상한만) | §9.5 Φ(CDF) 매핑은 정규분포를 균등분포로 바꾼다. 균등분포에서 상위 10%가 90점을 넘는 것이 정상이다 |
| 10점 이하 비율 | **12% + 2×표본흔들림 이하** (상한만) | 위와 같음(하위 꼬리) |

> **(2026-09-07 개정) 꼬리 상한을 표본 수에 연동한다.**
>
> ```
> 상한       = 12% + 2 × 표본흔들림
> 표본흔들림 = sqrt(0.10 × 0.90 / N) × 100
> ```
>
> 근거: 상한을 고정하면 표본이 적을 때 정상 실행이 계속 걸린다. 실제로
> V11 허용폭을 두 차례 넓힌 것과 같은 반복이 된다. 표본이 늘면 상한이
> 자동으로 좁혀지므로 검사가 느슨해지지 않는다.
>
> | 채점 건수 | 표본흔들림 | 상한 |
> | --: | --: | --: |
> | 50 | 4.2%p | 20.5% |
...
> | 5,000 | 0.4%p | 12.8% |
>
> 리포트에는 상한과 표본 수를 함께 표시한다.
> `90점 이상  21곳 (12.6%)   정상: 16.6% 이하 (167곳 기준)   ✓`
