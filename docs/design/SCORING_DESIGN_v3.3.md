...
2. 점수가 제대로 퍼져 있나
  ...
  중간 점수      51점        (정상: 47~53점)     ✓
  90점 이상      15곳 (8.3%)  (정상: 8~12%)      ✓
  10점 이하      12곳 (6.6%)  (정상: 8~12%)      ✕
...
| :-- | :-- | :-- |
| 중앙값 | 47 ~ 53 | 유지 |
| 90점 이상 비율 | **8 ~ 12%** | §9.5 Φ(CDF) 매핑은 정규분포를 균등분포로 바꾼다. 균등분포에서 상위 10%가 90점을 넘는 것이 정상이다 |
| 10점 이하 비율 | **8 ~ 12%** | 위와 같음(하위 꼬리) |

> 개정 전 기준은 상·하위 모두 `0~2%` 였다. 이는 점수를 원점수 그대로 쓰는
> 전제의 값으로, Φ 매핑과 양립하지 않는다. 매핑을 쓰는 한 상·하위 10% 부근이
> 나오는 것이 정상이며, 옛 기준을 그대로 두면 정상 실행에도 매번 ✕ 가 찍힌다.
>
> 표본이 작으면 꼬리 비율의 흔들림이 커진다(예: 181건이면 10% 꼬리의
> 표준편차가 약 2.2%p). 범위를 벗어나면 우선 표본 수를 함께 확인한다.
...
| 5 | 점수 중앙값이 50 ± 3 밖, 또는 90점 이상/10점 이하 비율이 8~12% 밖 | "점수가 한쪽으로 몰려 있습니다" |
