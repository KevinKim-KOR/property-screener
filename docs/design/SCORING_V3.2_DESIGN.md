...
| **C12** | **발주자에게 SQL·로그·코드·스택트레이스를 제시하지 않는다.** 진단이 필요하면 구현 담당이 직접 실행하고 §19.0 리포트로 변환한다. "이 쿼리를 돌려보세요"는 그 자체로 결함이다 | 역할 분리 |
...
| `V8` | `total_households < 50` | 제외 | 나홀로 아파트. 비교군 통계가 성립하지 않음 |
...
**Level 1 카드에 허용되는 요소는 아래 10개뿐이다. 늘리지 않는다.**

| 요소 | 개수 |
| :-- | :--: |
| 단지명 | 1 |
| 위치 + 평형 + 층 (평형은 한글 통칭) | 1줄 |
| 점수 + 순위 설명 | 1 |
| 호가 | 1 |
| 최근 실거래가 | 1 |
| 가격 해석 문장 | 1 |
...
| 요소 | 사유 |
| :-- | :-- |
| 배지·태그 | 공간 대비 정보량 부족. 긍정 배지만 달리면 확증편향 유발 |
| 이모지 | 정보 밀도 저하 |
...
정합성  스코어링  파이프라인  마이그레이션  robust  MAD  IC
...
2. 점수가 제대로 퍼져 있나
  ...
  중간 점수      51점        (정상: 47~53점)     ✓
  90점 이상       8곳 (1.9%)  (정상: 0~2%)       ✓
  10점 이하      11곳 (2.6%)  (정상: 0~2%)       ✓
...
| 5 | 점수 중앙값이 50 ± 3 밖 | "점수가 한쪽으로 몰려 있습니다" |
