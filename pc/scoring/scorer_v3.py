...
def verify_run_level_gates(scored, base_date: str,
                           median_center: float = 50.0, median_tol: float = 5.0,
                           top_ratio: float = 0.10) -> None:
    """
    설계서 §11.5.2 run 단위 검증.

      V10  4개 블록 점수가 전부 상위 10%인 매물이 존재  -> run 실패 (정규화 미작동)
      V11  점수 중앙값이 50 ± 5 밖                      -> run 실패

    (2026-08-28 개정) 허용폭을 50±3 -> 50±5 로 넓혔다.
    붕괴 검출은 V10(4블록 전부 상위 10퍼센트)과 꼬리 비율 검사가 담당한다.
    중앙값은 Φ(CDF) 매핑의 성질상 원점수가 한쪽으로 치우치면 50 에서 벗어난다.
    실제로 기간 창을 base_date 상대로 바로잡은 뒤 중앙값이 46.6 이 되었는데,
